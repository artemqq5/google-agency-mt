...
REGISTER-SUCCESS = Приветствуем! 👋 Вы попали в MT Agency — сервис, созданный командой Masons Traffic специально для работы с агентскими кабинетами Google
REGISTER-FAIL = ❌ Ошибка регистрации

MENU = 📋 Меню
BACK = 🔙 Назад

CLIENT-ACCOUNT-NOT_EXIST = Аккаунта не существует

FAQ = ℹ️ FAQ
OPEN = Открыть
FAQ_MESSAGE = Инструкция для работы с агентскими аккаунтами MT Agency Google
...
CLIENT-MCC-BALANCE-HASH-SEND = 📄❗ <b>отправьте хеш транзакции</b>:
CLIENT-MCC-BALANCE-HASH = 📄 Пополните баланс на <b>{$sum}$</b>:

    ❗При пополнении меньше указанной суммы оплата не будет произведена автоматически❗
...
CLIENT-ACCOUNT-REFUND-CONFIRMATION-WARNING = Вы хотите сделать рефаунд аккаунта <b>{$account_name}</b>?
