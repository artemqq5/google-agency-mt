...
class BalanceRepository(DefaultDataBase):
    ...
    def balance_with_lock(self, mcc_uuid, team_uuid):
        query = "SELECT * FROM `balances` WHERE `mcc_uuid` = %s AND `team_uuid` = %s FOR UPDATE;"
        return self._select_one_tran(query, (mcc_uuid, team_uuid))
